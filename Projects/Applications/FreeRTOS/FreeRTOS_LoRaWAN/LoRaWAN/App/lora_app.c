...
static void SendTxData(void)
{
    ...
    // Turn on the LED
	  Pressure = get_data(800, 1100);
	  HAL_Delay(100);
	  Gaz = get_data(30, 200);
	  HAL_Delay(100);
	  Humidity = get_data(10, 100);
	  HAL_Delay(100);
	  Temperature = get_data(3, 100);
	...
	int16_t gaz = (uint16_t) Gaz;
	int16_t temperature = (uint16_t) Temperature;
	int16_t humidity = (uint16_t) Humidity;
	int16_t pressure = (uint16_t) Pressure;
